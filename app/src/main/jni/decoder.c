        ...
        if (hasInput) {
            ...
            if (bufIdx >= 0) {
                ...
                if (written < 0) {
                    debugLog("input EOS");
                    hasInput = false;
                    status = AMediaCodec_queueInputBuffer(codec, bufIdx, 0, 0, time,
...
bool decoder_dequeueBuffer(AMediaCodec *codec,
    ...
    } else if (bufIdx == AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED) {// More or less only for debugging
        AMediaFormat *format = AMediaCodec_getOutputFormat(codec);
        debugLog("Output format changed to: %s", AMediaFormat_toString(format));
        AMediaFormat_delete(format);
    }
    return !(info.flags & AMEDIACODEC_BUFFER_FLAG_END_OF_STREAM);
}
